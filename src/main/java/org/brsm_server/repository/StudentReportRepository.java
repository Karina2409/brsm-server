...
public interface StudentReportRepository extends JpaRepository<Report, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO student_report (student_id, document_id, opt) VALUES (:studentId, :reportId, :optCount)", nativeQuery = true)
    void addStudentToReport(@Param("studentId") Long studentId, @Param("reportId") Long reportId, @Param("optCount") Integer optCount);

    @Query("SELECT s FROM Student s JOIN s.reports r WHERE r.documentId = :reportId")
    Set<Student> findStudentsByReportId(@Param("reportId") Long reportId);

    @Query(value = "SELECT opt FROM student_report WHERE student_id = :studentId AND document_id = :reportId", nativeQuery = true)
    Integer findOptByStudentId(@Param("studentId") Long studentId, @Param("reportId") Long reportId);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM student_report WHERE student_id = :studentId AND document_id = :reportId", nativeQuery = true)
